...
if ($rights == 4 || $rights >= 6) {
    ...
    if (isset ($_POST['submit'])) {
        ...
        if ($mk == true) {
            ...
            $db->query("INSERT INTO `download` SET
              `refid` = $cat,
              `adres` = '" . $db->real_escape_string($droot) . "',
              `time` = " . time() . ",
              `name` = '$drn',
              `type` = 'cat',
              `ip` = '',
              ...
              `screen` = ''
              ");
            $categ = $db->query("select * from `download` where type = 'cat' and name='$drn' and refid = '" . $cat . "';");
            $newcat = $categ->fetch_array();
