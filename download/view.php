...
$file2 = $file1->num_rows;
$adrfile = $file1->fetch_array();
...
$dnam = $db->query("select * from `download` where type = 'cat' and id = '" . $adrfile['refid'] . "';");
$dnam1 = $dnam->fetch_array();
$dirname = "$dnam1[text]";
$dirid = "$dnam1[id]";
$nadir = $adrfile['refid'];
...
while ($nadir != "" && $nadir != "0") {
    ...
    $dnamm = $db->query("select * from `download` where type = 'cat' and id = '" . $nadir . "';");
    $dnamm1 = $dnamm->fetch_array();
    $dnamm2 = $db->query("select * from `download` where type = 'cat' and id = '" . $dnamm1['refid'] . "';");
    $dnamm3 = $dnamm2->fetch_array();
    $nadir = $dnamm1['refid'];
    $dirname = $dnamm3['text'];
}
...
if ($set['mod_down_comm'] || $rights >= 7) {
    $totalkomm = $db->query("SELECT COUNT(*) FROM `download` WHERE `type` = 'komm' AND `refid` = '$file'")->fetch_row()[0];
