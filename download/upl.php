...
<a href='?act=select&amp;cat=" . $cat . "'>Повторить</a><br/>";
        ...
        if ((move_uploaded_file($_FILES["fail"]["tmp_name"], "$loaddir/$newname")) == true) {
            ...
            $db->query("insert into `download` values(0,'" . $cat . "','" . $loaddir . "','" . time() . "','" . $ch . "','file','','','','" . $opis . "','" . (isset($ch1) ? $ch1 : '') . "');");
        }
        else {
...
<a href='?act=select&amp;cat=" . $cat . "'>Повторить</a><br/>";
                ...
                $ch = "$newname.$ftip";
                $db->query("insert into `download` values(0,'" . $cat . "','" . $loaddir . "','" . time() . "','" . $ch . "','file','','','','" . $opis . "','" . $ch1 . "');");
            }
            else {
                echo 'Ошибка при загрузке файла<br/>';
            }
