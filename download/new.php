...
$req = $db->query("SELECT COUNT(*) FROM `download` WHERE `time` > '" . (time() - 259200) . "' AND `type` = 'file'");
...
if ($total) {
    ...
    while ($newf = $req->fetch_array()) {
        ...
        $pat = "";
        while ($nadir != "") {
            $dnew = $db->query("select * from `download` where type = 'cat' and id = '" . $nadir . "'");
            $dnew1 = $dnew->fetch_array();
            $pat = "$dnew1[text]/$pat";
            $nadir = $dnew1['refid'];
        }
        $l = mb_strlen($pat);
