...
if (in_array($act, $array)) {
    ...
    echo '<p><a href="?act=new">' . $lng['new_files'] . '</a> (' . $db->query("SELECT COUNT(*) FROM `download` WHERE `time` > '" . (time() - 259200) . "' AND `type` = 'file'")->fetch_row()[0] . ')</p>';
    ...
    if (empty($_GET['cat'])) {
        ...
        $res = $req->fetch_array();
        if ($req->num_rows == 0 || !is_dir($res['adres'] . '/' . $res['name'])) {
        ...
        $dirid = $cat;
        while ($dirid != '0' && $dirid != "") {
            $req = $db->query("SELECT * FROM `download` WHERE `type` = 'cat' and `id` = '" . $dirid . "' LIMIT 1");
            $res = $req->fetch_array();
    ...
    $req = $db->query("SELECT COUNT(*) FROM `download` WHERE `refid` = '$cat' AND `type` = 'cat'");
    $totalcat = $req->fetch_row()[0];
    // Подсчитываем число файлов
    $req = $db->query("SELECT COUNT(*) FROM `download` WHERE `refid` = '$cat' AND `type` = 'file'");
    $totalfile = $req->fetch_row()[0];
    ...
    if ($total > 0) {
        ...
        while ($zap2 = $zap->fetch_array()) {
            ...
            if ($totalcat > 0 && $zap2['type'] == 'cat') {
                ...
                $req = $db->query("SELECT COUNT(*) FROM `download` WHERE `type` = 'file' AND `adres` LIKE '" . ($zap2['adres'] . '/' . $zap2['name']) . "%'");
                $g = $req->fetch_row()[0];
                // Считаем новые файлы в подкаталогах
                $req = $db->query("SELECT COUNT(*) FROM `download` WHERE `type` = 'file' AND `adres` LIKE '" . ($zap2['adres'] . '/' . $zap2['name']) . "%' AND `time` > '" . (time() - 259200) . "'");
                $g1 = $req->fetch_row()[0];
                echo "($g";
                if ($g1 != 0) {
                    echo "/+$g1)</div>";
