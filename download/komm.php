...
$countm = $mess->num_rows;

$fayl1 = $fayl->fetch_array();
...
while ($mass = $mess->fetch_array()) {
    if ($i >= $start && $i < $end) {
        ...
        $uz = @ $db->query("SELECT * FROM `users` WHERE name='" . functions::check($mass[avtor]) . "';");
        $mass1 = @ $uz->fetch_array();
        echo "$div";
