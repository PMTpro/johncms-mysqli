    ...
    case 'sent':
        ...
        */
        ...
        if (!$error) {
            ...
            $req = $db->query("SELECT * FROM `users` WHERE `name_lat` = '$nick' LIMIT 1");
            if ($req->num_rows == 1) {
                $res = $req->fetch_array();
        ...
        if (!$error) {
            ...
            if (mail($res['mail'], $subject, $mail, $adds)) {
                $db->query("UPDATE `users` SET `rest_code` = '" . $check_code . "', `rest_time` = '" . time() . "' WHERE `id` = '" . $res['id'] . "'");
        ...
        */
        ...
        if ($req->num_rows) {
            ...
            if (!$error && ($res['rest_time'] < time() - 3600 || $code != $res['rest_code'])) {
                ...
                $db->query("UPDATE `users` SET `rest_code` = '', `rest_time` = '' WHERE `id` = '$id'");
            }
        } else {
            $error = $lng['error_user_not_exist'];
        }
        if (!$error) {
            ...
            $adds .= "Content-Type: text/plain; charset=\"utf-8\"\r\n";
            if (mail($res['mail'], $subject, $mail, $adds)) {
                $db->query("UPDATE `users` SET `rest_code` = '', `password` = '" . md5(md5($pass)) . "' WHERE `id` = '$id'");
