...
if ($search && !$error) {
    ...
    */
    ...
    $search_db = '%' . $search_db . '%';
    $total = $db->query("SELECT COUNT(*) FROM `users` WHERE `name_lat` LIKE '" . $db->real_escape_string($search_db) . "'")->fetch_row()[0];
    ...
    if ($total > 0) {
        $req = $db->query("SELECT * FROM `users` WHERE `name_lat` LIKE '" . $db->real_escape_string($search_db) . "' ORDER BY `name` ASC LIMIT $start, $kmess");
        $i = 0;
