...
if (array_key_exists($act, $array) && file_exists($path . $act . '.php')) {
    ...
    */
    $textl = $lng['community'];
    require('../incfiles/head.php');
    $brth = $db->query("SELECT COUNT(*) FROM `users` WHERE `dayb` = '" . date('j', time()) . "' AND `monthb` = '" . date('n', time()) . "' AND `preg` = '1'")->fetch_row()[0];
    $count_adm = $db->query("SELECT COUNT(*) FROM `users` WHERE `rights` > 0")->fetch_row()[0];
