...
if (array_key_exists($act, $array) && file_exists($path . $act . '.php')) {
    ...
    require('../incfiles/head.php');
    $albumcount = $db->query("SELECT COUNT(DISTINCT `user_id`) FROM `cms_album_files`")->fetch_row()[0];
    ...
    $total_womans = $db->query("SELECT COUNT(DISTINCT `user_id`)
      ...
      WHERE `users`.`sex` = 'zh'
    ")->fetch_row()[0];
    $newcount = $db->query("SELECT COUNT(*) FROM `cms_album_files` WHERE `time` > '" . (time() - 259200) . "' AND `access` > '1'")->fetch_row()[0];
