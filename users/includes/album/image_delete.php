...
if ($img && $user['id'] == $user_id || $rights >= 6) {
    ...
    if ($req->num_rows) {
        ...
        if (isset($_POST['submit'])) {
            ...
            $db->query("DELETE FROM `cms_album_files` WHERE `id` = '$img'");
            $db->query("DELETE FROM `cms_album_votes` WHERE `file_id` = '$img'");
            $db->query("OPTIMIZE TABLE `cms_album_votes`");
            $db->query("DELETE FROM `cms_album_comments` WHERE `sub_id` = '$img'");
