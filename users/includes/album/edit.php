...
if ($user['id'] == $user_id && empty($ban) || $rights >= 7) {
    ...
    if (isset($_POST['submit'])) {
        ...
        if (!$al && $db->query("SELECT * FROM `cms_album_cat` WHERE `name` = '" . $db->real_escape_string($name->num_rows . "' AND `user_id` = '" . $user['id'] . "' LIMIT 1")))
            $error[] = $lng_profile['error_album_exists'];
        if (!$error) {
            if ($al) {
                // Изменяем данные в базе
                $db->query("UPDATE `cms_album_files` SET `access` = '$access' WHERE `album_id` = '$al' AND `user_id` = '" . $user['id'] . "'");
                $db->query("UPDATE `cms_album_cat` SET
                ...
                $db->query("INSERT INTO `cms_album_cat` SET
                    `user_id` = '" . $user['id'] . "',
                    `name` = '" . $db->real_escape_string($name) . "',
                    `description` = '" . $db->real_escape_string($description) . "',
                    `password` = '" . $db->real_escape_string($password) . "',
