...
switch ($mod) {
    ...
    case 'last_comm':
        ...
        -----------------------------------------------------------------
        */
        $total = $db->query("SELECT COUNT(DISTINCT `sub_id`) FROM `cms_album_comments` WHERE `time` >" . (time() - 86400))->fetch_row()[0];
...
if ($mod == 'my_new_comm') {
    ...
    $total = $db->query("SELECT COUNT(*) FROM `cms_album_files` WHERE $where")->fetch_row()[0];
}
