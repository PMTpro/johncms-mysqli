    ...
    if ($req_a->num_rows) {
        ...
        if (isset($_POST['submit'])) {
            ...
            while ($res = $req->fetch_assoc()) {
                ...
                $db->query("DELETE FROM `cms_album_votes` WHERE `file_id` = '" . $res['id'] . "'");
                // Удаляем комментарии
                $db->query("DELETE FROM `cms_album_comments` WHERE `sub_id` = '" . $res['id'] . "'");
            }
            // Удаляем записи из таблиц
            $db->query("DELETE FROM `cms_album_files` WHERE `album_id` = '" . $res_a['id'] . "'");
            $db->query("DELETE FROM `cms_album_cat` WHERE `id` = '" . $res_a['id'] . "'");
            $db->query("OPTIMIZE TABLE `cms_album_cat`");
            $db->query("OPTIMIZE TABLE `cms_album_votes`");
            $db->query("OPTIMIZE TABLE `cms_album_files`");
            $db->query("OPTIMIZE TABLE `cms_album_comments`");
