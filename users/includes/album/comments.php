...
if ($req_obj->num_rows) {
    ...
    */
    unset($_SESSION['ref']);
    $req_a = $db->query("SELECT * FROM `cms_album_cat` WHERE `id` = '" . $res_obj['album_id'] . "'");
    ...
    */
    if(core::$user_id == $owner['id'] && $res_obj['unread_comments'])
        $db->query("UPDATE `cms_album_files` SET `unread_comments` = '0' WHERE `id` = '$img' LIMIT 1");
    ...
    */
    if($comm->added && core::$user_id != $owner['id'])
        $db->query("UPDATE `cms_album_files` SET `unread_comments` = '1' WHERE `id` = '$img' LIMIT 1");
} else {
