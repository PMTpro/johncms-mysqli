...
$total_photo = $db->query("SELECT COUNT(*) FROM `cms_album_files` WHERE `user_id` = '$user_id'")->fetch_row()[0];
$total_friends = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `user_id`='$user_id' AND `type`='2' AND `friends`='1'")->fetch_row()[0];
$new_friends = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `from_id`='$user_id' AND `type`='2' AND `friends`='0';")->fetch_row()[0];
$online_friends = $db->query("SELECT COUNT(*) FROM `cms_contact` LEFT JOIN `users` ON `cms_contact`.`from_id`=`users`.`id` WHERE `cms_contact`.`user_id`='$user_id' AND `cms_contact`.`type`='2' AND `cms_contact`.`friends`='1' AND `lastdate` > " . (time() - 300) . "")->fetch_row()[0];
echo '' .
...
$count_input = $db->query("
	...
	AND `cms_contact`.`ban`!='1' AND `spam`='0'")->fetch_row()[0];
...
WHERE `cms_mail`.`user_id`='$user_id' AND `cms_mail`.`delete`!='$user_id' AND `cms_mail`.`sys`='0' AND `cms_contact`.`ban`!='1'")->fetch_row()[0];
...
WHERE `cms_mail`.`user_id`='$user_id' AND `cms_mail`.`delete`!='$user_id' AND `cms_mail`.`read`='0' AND `cms_mail`.`sys`='0' AND `cms_contact`.`ban`!='1'")->fetch_row()[0];
...
WHERE `from_id`='$user_id' AND `delete`!='$user_id' AND `sys`='1'")->fetch_row()[0];
//Системные сообщения
$count_systems_new = $db->query("SELECT COUNT(*) FROM `cms_mail`
WHERE `from_id`='$user_id' AND `delete`!='$user_id' AND `sys`='1' AND `read`='0'")->fetch_row()[0];
...
WHERE (`user_id`='$user_id' OR `from_id`='$user_id') AND `delete`!='$user_id' AND `file_name`!='';")->fetch_row()[0];
...
WHERE `user_id`='" . $user_id . "' AND `ban`!='1';")->fetch_row()[0];
