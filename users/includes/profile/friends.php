...
if ($id && $id != $user_id && $do) {
    ...
    $result = $contacts->fetch_assoc();

    switch ($do) {
        case 'add':
            $fr = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `type`='2' AND ((`from_id`='$id' AND `user_id`='$user_id') OR (`from_id`='$user_id' AND `user_id`='$id'))")->fetch_row()[0];
            if ($fr != 2) {
                if (isset($_POST['submit'])) {
                    $fr_out = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `type`='2' AND `user_id`='$user_id' AND `from_id`='$id'")->fetch_row()[0];
                    if ($fr_out) {
                    ...
                    $fr_in = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `type`='2' AND `from_id`='$user_id' AND `user_id`='$id'")->fetch_row()[0];
                    if ($fr_in) {
                    ...
                    $my = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `user_id`='$user_id' AND `from_id`='$id'")->fetch_row()[0];

                    if ($my != 0) {
                        $db->query("UPDATE `cms_contact` SET
        ...
        case 'cancel':
            ...
            if ($fr != 2) {
                if (isset($_POST['submit'])) {
                    ...
                    $db->query("DELETE FROM `cms_mail` WHERE `text`='" . $db->real_escape_string($text) . "'");
        ...
        case 'okfriends':
            if ($set_mail['cat_friends']) {
                $fr = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `user_id`='$user_id' AND `from_id`='$id'")->fetch_row()[0];

                if ($fr) {
                    $cont = $db->query("SELECT * FROM `cms_contact` WHERE `user_id`='$id' AND `from_id`='$user_id'");
                    $res = $cont->fetch_assoc();
                    if (isset($_POST['submit'])) {
                        $arr_fr = array(
                            1 => $lng_profile['you_my_friends'],
                            $lng_profile['you_my_classfriends'],
                        ...
                        $db->query("DELETE FROM `cms_mail` WHERE `user_id` = '$id' AND `from_id` = '$user_id' AND `text`='" . $db->real_escape_string($text) . "'");
                        $arr_fr1 = array(
                        ...
                        $db->query("UPDATE `cms_contact` SET `man`='{$res['man']}' WHERE `user_id`='$user_id' AND `from_id`='$id'");
        ...
        case 'ok':
            ...
            if ($fr != 2) {
                if (isset($_POST['submit'])) {
                    $fr_out = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `type`='2' AND `user_id`='$id' AND `from_id`='$user_id'")->fetch_row()[0];
        ...
        case 'no':
            ...
            if ($fr != 2) {
                if (isset($_POST['submit'])) {
                    ...
                    $db->query("UPDATE `cms_contact` SET `type`='1' WHERE `user_id`='$id' AND `from_id`='$user_id'");
    ...
    if ($user['id'] && $user['id'] != $user_id) {
        ...
        $total = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `user_id`='{$user['id']}' AND `type`='2' AND `friends`='1' AND `ban`!='1'" . $sql)->fetch_row()[0];
        ...
        if ($total) {
            ...
            for ($i = 0; $row = $req->fetch_assoc(); ++$i) {
                ...
                if ($row['id'] == $user_id) {
                    $count_message = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE ((`user_id`='{$user['id']}' AND `from_id`='$user_id') OR (`user_id`='$user_id' AND `from_id`='{$user['id']}')) AND `sys`!='1' AND `delete`!='$user_id';")->fetch_row()[0];
                    $new_count_message = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE `cms_mail`.`from_id`='{$user['id']}' AND `cms_mail`.`user_id`='$user_id' AND `read`='0' AND `sys`!='1' AND `delete`!='$user_id';")->fetch_row()[0];
        ...
        switch ($do) {
            case 'demands':
                ...
                $total = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `user_id`='$user_id' AND `type`='2' AND `friends`='0' AND `ban`!='1'")->fetch_row()[0];
                ...
                if ($total) {
                    ...
                    for ($i = 0; $row = $req->fetch_assoc(); ++$i) {
                        ...
                        $count_message = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE ((`user_id`='{$row['id']}' AND `from_id`='$user_id') OR (`user_id`='$user_id' AND `from_id`='{$row['id']}')) AND `delete`!='$user_id';")->fetch_row()[0];
                        $new_count_message = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE `cms_mail`.`from_id`='{$row['id']}' AND `cms_mail`.`user_id`='$user_id' AND `read`='0' AND `delete`!='$user_id';")->fetch_row()[0];
            ...
            case 'offers':
                ...
                $total = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `from_id`='$user_id' AND `type`='2' AND `friends`='0' AND `ban`!='1'")->fetch_row()[0];
                ...
                if ($total) {
                    ...
                    for ($i = 0; $row = $req->fetch_assoc(); ++$i) {
                        ...
                        $count_message = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE ((`user_id`='{$row['from_id']}' AND `from_id`='$user_id') OR (`user_id`='$user_id' AND `from_id`='{$row['user_id']}')) AND `delete`!='$user_id' AND `spam`='0' AND `sys`='0';")->fetch_row()[0];
                        $new_count_message = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE `cms_mail`.`from_id`='{$row['from_id']}' AND `cms_mail`.`user_id`='$user_id' AND `read`='0' AND `delete`!='$user_id' AND `spam`='0' AND `sys`='0';")->fetch_row()[0];
                        $arg = array(
                            'header' => '(' . $count_message . ($new_count_message ? '/<span class="red">+' . $new_count_message . '</span>' : '') . ')',
                            'sub'    => $subtext
                        );
            ...
            case 'online':
                ...
                if ($total) {
                    ...
                    for ($i = 0; $row = $req->fetch_assoc(); ++$i) {
                        ...
                        $count_message = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE ((`user_id`='{$row['id']}' AND `from_id`='$user_id') OR (`user_id`='$user_id' AND `from_id`='{$row['id']}')) AND `sys`!='1' AND `spam`!='1' AND `delete`!='$user_id';")->fetch_row()[0];
                        $new_count_message = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE `cms_mail`.`user_id`='{$row['id']}' AND `cms_mail`.`from_id`='$user_id' AND `read`='0' AND `sys`!='1' AND `spam`!='1' AND `delete`!='$user_id';")->fetch_row()[0];
                        $arg = array(
                            'header' => '(' . $count_message . ($new_count_message ? '/<span class="red">+' . $new_count_message . '</span>' : '') . ')',
                            'sub'    => $subtext
                        );
            ...
            default:
                $off = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `from_id`='$user_id' AND `type`='2' AND `friends`='0' AND `ban`='0'")->fetch_row()[0];
                $dem = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `user_id`='$user_id' AND `type`='2' AND `friends`='0' AND `ban`='0'")->fetch_row()[0];
