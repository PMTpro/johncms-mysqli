        ...
        */
        if ($rights < 1 || ($rights < 6 && $user['rights']) || ($rights <= $user['rights'])) {
            ...
            if (isset($_POST['submit'])) {
                ...
                if ($db->query("SELECT COUNT(*) FROM `cms_ban_users` WHERE `user_id` = '" . $user['id'] . "' AND `ban_time` > '" . time() . "' AND `ban_type` = '$term'")->fetch_row()[0])
                ...
                if (!$error) {
                    // Заносим в базу
                    $db->query("INSERT INTO `cms_ban_users` SET
                        `user_id` = '" . $user['id'] . "',
                        `ban_time` = '" . (time() + $timeval) . "',
                        `ban_while` = '" . time() . "',
                        `ban_type` = '$term',
                        `ban_who` = '$login',
                        `ban_reason` = '" . $db->real_escape_string($reason) . "'
                    ");
                    if ($set_karma['on']) {
        ...
        if (!$ban || $user['id'] == $user_id || $rights < 7)
            ...
            if ($req->num_rows) {
                ...
                if (!$error) {
                    ...
                    if (isset($_POST['submit'])) {
                        $db->query("UPDATE `cms_ban_users` SET `ban_time` = '" . time() . "' WHERE `id` = '$ban'");
        ...
        */
        if (!$ban || $rights < 9)
            ...
            if ($req->num_rows) {
                ...
                if (isset($_POST['submit'])) {
                    $db->query("DELETE FROM `karma_users` WHERE `karma_user` = '" . $user['id'] . "' AND `user_id` = '0' AND `time` = '" . $res['ban_while'] . "' LIMIT 1");
                    $points = $set_karma['karma_points'] * 2;
                    $db->query("UPDATE `users` SET
                        `karma_minus` = '" . ($user['karma_minus'] > $points ? $user['karma_minus'] - $points : 0) . "'
                        WHERE `id` = '" . $user['id'] . "'
                    ");
                    $db->query("DELETE FROM `cms_ban_users` WHERE `id` = '$ban'");
        ...
        */
        if ($rights == 9) {
            echo '<div class="phdr"><b>' . $lng_ban['infringements_history'] . '</b></div>' .
                 '<div class="gmenu"><p>' . functions::display_user($user) . '</p></div>';
            if (isset($_POST['submit'])) {
                $db->query("DELETE FROM `cms_ban_users` WHERE `user_id` = '" . $user['id'] . "'");
            ...
            $total = $db->query("SELECT COUNT(*) FROM `cms_ban_users` WHERE `user_id` = '" . $user['id'] . "'")->fetch_row()[0];
