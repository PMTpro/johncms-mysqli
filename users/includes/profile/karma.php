    ...
    switch ($mod) {
            ...
            if (!$datauser['karma_off'] && !$ban) {
                ...
                if ($error) {
                    ...
                    if (isset($_POST['submit'])) {
                        $text = isset($_POST['text']) ? $db->real_escape_string(mb_substr(trim($_POST['text']), 0, 500)) : '';
                        $type = intval($_POST['type']) ? 1 : 0;
                        $points = abs(intval($_POST['points']));
                        if (!$points || $points > ($set_karma['karma_points'] - $sum))
                            $points = 1;
                        $db->query("INSERT INTO `karma_users` SET
                            `user_id` = '$user_id',
                            `name` = '$login',
                            `karma_user` = '" . $user['id'] . "',
                            `points` = '$points',
            ...
            */
            if ($rights == 9) {
                ...
                if ($req->num_rows) {
                    $res = $req->fetch_assoc();
                    if (isset($_GET['yes'])) {
                        $db->query("DELETE FROM `karma_users` WHERE `id` = '$id'");
        ...
        case 'clean':
            ...
            */
            if ($rights == 9) {
                if (isset($_GET['yes'])) {
                    $db->query("DELETE FROM `karma_users` WHERE `karma_user` = '" . $user['id'] . "'");
                    $db->query("OPTIMIZE TABLE `karma_users`");
                    $db->query("UPDATE `users` SET `karma_plus` = '0', `karma_minus` = '0' WHERE `id` = '" . $user['id'] . "'");
        ...
        case 'new':
            ...
            */
            ...
            $total = $db->query("SELECT COUNT(*) FROM `karma_users` WHERE `karma_user` = '$user_id' AND `time` > " . (time() - 86400))->fetch_row()[0];
            if ($total) {
        ...
        default:
            ...
            */
            ...
            $total = $db->query("SELECT COUNT(*) FROM `karma_users` WHERE `karma_user` = '" . $user['id'] . "'" . ($type == 2 ? "" : " AND `type` = '$type'"))->fetch_row()[0];
            if ($total) {
