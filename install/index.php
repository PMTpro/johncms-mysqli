...
class install
{
    ...
    static function check_php_errors()
    {
        ...
        if (!extension_loaded('mysqli')) $error[] = 'mysqli';
    ...
    case 'set':
        ...
        */
        ...
        if (isset($_POST['check']) || isset($_POST['install'])) {
            ...
            if (empty($db_error)) {
                $con_err = false;
                $check = @new mysqli($db_host, $db_user, $db_pass);
                $con_err = $check->connect_error;
            ...
            if (empty($db_error) && $check->select_db($db_name) == false) {
        ...
        if ($db_check && isset($_POST['install'])) {
            ...
            if ($db_check && empty($site_error) && empty($admin_error)) {
                ...
                $db = new mysqli($db_host, $db_user, $db_pass) or die('ERROR: cannot connect to DB server</body></html>');
                $db->select_db($db_name) or die('ERROR: cannot select DB</body></html>');
                ...
                $db->query("UPDATE `cms_settings` SET `val`='$language' WHERE `key`='lng'");
                $db->query("UPDATE `cms_settings` SET `val`='" . $db->real_escape_string($site_url) . "' WHERE `key`='homeurl'");
                $db->query("UPDATE `cms_settings` SET `val`='" . $db->real_escape_string($site_mail) . "' WHERE `key`='email'");

                // Создаем Администратора
                $db->query("INSERT INTO `users` SET
                    `name` = '" . $db->real_escape_string($admin_user) . "',
                    `name_lat` = '" . $db->real_escape_string(mb_strtolower($admin_user)) . "',
                    `password` = '" . md5(md5($admin_pass)) . "',
                    `sex` = 'm',
                    `datereg` = '" . time() . "',
                    `lastdate` = '" . time() . "',
                    `mail` = '" . $db->real_escape_string($site_mail) . "',
                    `www` = '" . $db->real_escape_string($site_url) . "',
                    `about` = '',
                    `set_user` = '',
                    `set_forum` = '',
                    `set_mail` = '',
                    ...
                    `browser` = '" . $db->real_escape_string(htmlentities($_SERVER["HTTP_USER_AGENT"])) . "',
                    `preg` = '1'
                ") or die('ERROR: Administrator setup<br/>" . $db- >error . "</body></html>');
