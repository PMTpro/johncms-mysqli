...
// config
$sql = $db->query("SELECT `id`, `pos` FROM `library_cats` WHERE " . ($do == 'dir' ? '`parent`=' . $id : '`parent`=0') . " ORDER BY `pos` ASC");
$y = 0;
$arrsort = array();

if ($db->query("SELECT COUNT(*) FROM `library_cats` WHERE " . ($do == 'dir' ? '`parent`=' . $id : '`parent`=0'))->fetch_row()[0]) {
...
class Tree
{
    ...
    */
    public function get_all_childs_id($id = 0)
    {
        ...
        $dirtype = core::$db->query("SELECT COUNT(*) FROM `library_cats` WHERE `id` = " . $id)->fetch_row()[0] ? core::$db->query("SELECT `dir` FROM `library_cats` WHERE `id` = " . $id . " LIMIT 1")->fetch_row()[0] : 0;
        if ($dirtype) {
            $sql = core::$db->query("SELECT `id` FROM `library_cats` WHERE `parent`=" . $id);
            $this->result['dirs'][$id] = $id;
        } else {
            $this->result['dirs'][$id] = $id;
            $sql = core::$db->query("SELECT `id` FROM `library_texts` WHERE `cat_id`=" . $id);
        }
        if ($sql->num_rows > 0) {
            while ($this->child = $sql->fetch_assoc()) {
    ...
    */
    public function clean_trash($data) {
        if (!is_array($data)) {
            core::$db->query("DELETE FROM `cms_library_comments` WHERE `sub_id` = " . $data);
    ...
    */
    public function clean_dir()
    {
        ...
        core::$db->query("DELETE FROM `library_cats` WHERE `id` IN(" . implode(', ', $dirs) . ")");
        $dirs = mysql_affected_rows();
        core::$db->query("DELETE FROM `library_texts` WHERE `id` IN(" . implode(', ', $texts) . ")");
    ...
    */
    public function get_childs_dir($parent = 0)
    {
        ...
        $sql = core::$db->query("SELECT `id` FROM `library_cats` WHERE `parent`=" . $parent . " AND `dir`=1");
        if ($sql->num_rows > 0) {
            while ($this->child = $sql->fetch_assoc()) {
    ...
    */
    ...
    {
        $id = $id == 0 ? $this->start_id : $id;
        $this->parent = core::$db->query("SELECT `id`, `name`, `parent` FROM `library_cats` WHERE id='" . $id . "' LIMIT 1")->fetch_assoc();
...
class Hashtags
{
    ...
    {
        $cnt = core::$db->query("SELECT COUNT(*) FROM `library_tags` WHERE `tag_name` = '" . core::$db->real_escape_string($tag) . "'")->fetch_row()[0];
        if ($cnt) {
            $res = array();
            $sql = core::$db->query("SELECT `lib_text_id` FROM `library_tags` WHERE `tag_name` = '" . core::$db->real_escape_string($tag) . "'");
            while ($row = $sql->fetch_assoc()) {
                $res[] = $row['lib_text_id'];
            }
    ...
    {
        $cnt = core::$db->query("SELECT COUNT(*) FROM `library_tags` WHERE `lib_text_id` = " . $this->lib_id)->fetch_row()[0];
        if ($cnt) {
            $res = array();
            $sql = core::$db->query("SELECT `tag_name` FROM `library_tags` WHERE `lib_text_id` = " . $this->lib_id);
            while ($row = $sql->fetch_assoc()) {
                $res[] = $row['tag_name'];
            }
    ...
    public function add_tags($tags)
    {
        if (empty($tags)) {
            ...
            foreach ($tags as $tag) {
                if (!$this->isset_tag($this->valid_tag($tag))) {
                    $array_res[] = '("' . $this->lib_id . '", "' . core::$db->real_escape_string($this->valid_tag($tag)) . '")';
                }
            }
        }
        if (sizeof($array_res) > 0) {
            ...
            return core::$db->query($res) ? mysql_affected_rows() : 0;
        } else {
    ...
    {
        core::$db->query("DELETE FROM `library_tags` WHERE `lib_text_id` = " . $this->lib_id);
        return mysql_affected_rows();
    }

    public function isset_tag($tag)
    {
        return $this->lib_id ? (core::$db->query("SELECT * FROM `library_tags` WHERE `lib_text_id` = " . $this->lib_id . " AND `tag_name` = '" . core::$db->real_escape_string($tag->num_rows . "'")) ? 1 : 0) : 0;
    }
    ...
    {
        $result = array();
        $sql = core::$db->query("SELECT `tag_name`, COUNT(*) as `count` FROM `library_tags` GROUP BY `tag_name` ORDER BY `count` DESC;");
        if ($sql->num_rows) {
            while($row = $sql->fetch_assoc()) {
    ...
    public function set_cache($sort = 'cmpalpha') {
        ...
        $obj = new self();
        $tags = core::$db->query("SELECT `id` FROM `library_tags` LIMIT 1")->num_rows;
...
class Rating
{
    ...
    public function add_vote($point) {
        ...
        if (core::$db->query("SELECT COUNT(*) FROM `cms_library_rating` WHERE `user_id` = " . $user_id . " AND `st_id` = " . $this->lib_id)->fetch_row()[0] > 0) {
            core::$db->query("UPDATE `cms_library_rating` SET `point` = " . $point . " WHERE `user_id` = " . $user_id . " AND `st_id` = " . $this->lib_id);
        } elseif ($user_id && $this->lib_id > 0) {
            core::$db->query("INSERT INTO `cms_library_rating` (`user_id`, `st_id`, `point`) VALUES (" . $user_id . ", " . $this->lib_id . ", " . $point . ")");
        }
    ...
    public function get_rate() {
        return floor(core::$db->query("SELECT AVG(`point`) FROM `cms_library_rating` WHERE `st_id` = " . $this->lib_id)->fetch_row()[0] * 2) / 2;
    }
    
    public function view_rate($anchor = 0) {
        $res = ($anchor ? '<a href="#rating">' : '') . functions::image('rating/star.' . (str_replace('.', '-', (string) $this->get_rate())) . '.gif', array('alt' => 'rating ' . $this->lib_id . ' article')) . ($anchor ? '</a>' : '') . ' (' . core::$db->query("SELECT COUNT(*) FROM `cms_library_rating` WHERE `st_id` = " . $this->lib_id)->fetch_row()[0] . ')';
    ...
    public function get_vote() {
        if ($this->user_vote == -2) {
            global $user_id;
            $this->user_vote = core::$db->query("SELECT COUNT(*) FROM `cms_library_rating` WHERE `user_id` = " . $user_id . " AND `st_id` = " . $this->lib_id)->fetch_row()[0] > 0 ? core::$db->query("SELECT `point` FROM `cms_library_rating` WHERE `user_id` = " . $user_id . " AND `st_id` = " . $this->lib_id . " LIMIT 1")->fetch_row()[0] : -1;
        }
