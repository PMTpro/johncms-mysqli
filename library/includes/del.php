...
$change = ($type == 'dir' ? $db->query("SELECT COUNT(*) FROM `library_cats` WHERE `parent`=" . $id) ->fetch_row()[0] > 0 || $db->query("SELECT COUNT(*) FROM `library_texts` WHERE `cat_id`=" . $id) ->fetch_row()[0] > 0 ? 0 : 1 : '');

switch ($type) {
case 'dir':
  if ($db->query("SELECT COUNT(*) FROM `library_cats` WHERE `id`=" . $id) ->fetch_row()[0] == 0) {
    echo functions::display_error($lng_lib['category_does_not_exist']);
  }
  elseif (!$change) {
    $mode = isset($_POST['mode']) ? $_POST['mode'] : (isset($do) ? $do : false);
    $dirtype = $db->query("SELECT `dir` FROM `library_cats` WHERE `id` = " . $id . " LIMIT 1")->fetch_row()[0];
    switch($mode) {
        case 'moveaction':
            if (!isset($_GET['movedeny'])) {
                ...
                if ($dirtype) {
                    $db->query("UPDATE `library_cats` SET `parent`=" . $move . " WHERE `parent` = " . $id);
                } else {
                    $db->query("UPDATE `library_texts` SET `cat_id` = " . $move . " WHERE `cat_id` = " . $id);
                }
                
                if (mysql_affected_rows()) {
                    $db->query("DELETE FROM `library_cats` WHERE `id` = " . $id);
                    if (mysql_affected_rows()) {
...
case 'article':
  if ($db->query("SELECT COUNT(*) FROM `library_texts` WHERE `id`=" . $id) ->fetch_row()[0] == 0) {
...
if (isset($_GET['yes']) && $type == 'image') {
  ...
  if ($db->query($sql)) {
   if ($type == 'article') {
    ...
    $db->query('DELETE FROM `library_tags` WHERE `lib_text_id` = "' . $id . '"');
   }
