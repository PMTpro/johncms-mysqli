...
$lng_gal = core::load_lng('gallery');
  ...
  if ($id && isset($_GET['yes'])) {
    ...
    echo '<div class="rmenu">' . $lng_lib['article'] . ' <strong>' . functions::checkout($db->query("SELECT `name` FROM `library_texts` WHERE `id`=" . $id) ->fetch_row()[0]) . '</strong> ' . $lng_lib['added_to_database'] . '</div>';
  }
  ...
  $total = $db->query('SELECT COUNT(*) FROM `library_texts` WHERE `premod`=0') ->fetch_row()[0];
  $page = $page >= ceil($total / $kmess) ? ceil($total / $kmess) : $page;
  $start = $page == 1 ? 0 : ($page - 1) * $kmess;
  if ($total) { 
    $sql = $db->query('SELECT `id`, `name`, `time`, `uploader`, `uploader_id`, `cat_id` FROM `library_texts` WHERE `premod`=0 ORDER BY `time` DESC LIMIT ' . $start . ',' . $kmess);
    $i = 0;
