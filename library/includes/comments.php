...
if ($req_obj->num_rows) {
    ...
    $obj = new Hashtags($id);
    $catalog = $db->query("SELECT `id`, `name` FROM `library_cats` WHERE `id`=" . $res_obj['cat_id'] . " LIMIT 1")->fetch_assoc();
    $context_top =
    ...
    if ($comm->added) {
        $db->query("UPDATE `library_texts` SET `count_comments`=" . ($res_obj['count_comments'] > 0 ? ++$res_obj['count_comments'] : 1) . " WHERE `id`=" . $id);
    }
} else {
