...
if ($db->query('SELECT COUNT(*) FROM `cms_library_comments`')->fetch_row()[0] > 0) {

$sql = $db->query('SELECT `cms_library_comments`.`user_id` , `cms_library_comments`.`text` , `library_texts`.`name` , `library_texts`.`count_comments` , `library_texts`.`id` , `cms_library_comments`.`time` 
...
$i = 0;
  while ($row = $sql->fetch_assoc()) {
    ...
    . '<div class="sub">' . $lng_lib['added'] . ': ' . functions::checkout($db->query("SELECT `name` FROM `users` WHERE `id` = " . $row['user_id'])->fetch_row()[0]) . ' (' . functions::display_date($row['time']) . ')</div>'
    . '</div>';
  }
} else {
