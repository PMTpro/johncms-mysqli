...
switch ($type) {
case 'txt':
  $out .= bbcode::notags($db->query("SELECT `text` FROM `library_texts` WHERE `id`=" . $id . " LIMIT 1") ->fetch_row()[0]);
  break;
...
case 'fb2':
  ...
  $out.= '<p>' . $db->query("SELECT `name` FROM `library_texts` WHERE `id`=" . $id . " LIMIT 1") ->fetch_row()[0] . '</p>' . PHP_EOL;
  $out.= '</title>' . PHP_EOL . '<section>';
  $out.= '<p>' . str_replace('<p></p>', '<empty-line/>', str_replace(PHP_EOL, '</p>' . PHP_EOL . '<p>', bbcode::notags($db->query("SELECT `text` FROM `library_texts` WHERE `id`=" . $id . " LIMIT 1") ->fetch_row()[0]))) . '</p>' . PHP_EOL;
