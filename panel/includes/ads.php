...
switch ($mod) {
    case 'edit':
        ...
        */
        ...
        if (isset($_POST['submit'])) {
            $link = isset($_POST['link']) ? $db->real_escape_string(trim($_POST['link'])) : '';
            ...
            if ($type > 3 || $type < 0)
                $type = 0;
            if (!$mesto) {
                $total = $db->query("SELECT COUNT(*) FROM `cms_ads` WHERE `mesto` = '" . $mesto . "' AND `type` = '" . $type . "'")->fetch_row()[0];
                if ($total != 0)
            ...
            if ($id) {
                ...
                $db->query("INSERT INTO `cms_ads` SET
                    `type` = '$type',
                    `view` = '$view',
                    `mesto` = '$mesto',
                    `link` = '$link',
        ...
        if ($id) {
            ...
            if ($req->num_rows > 0) {
                ...
                if ($req->num_rows > 0) {
                    ...
                    $id2 = $res['id'];
                    $mesto2 = $res['mesto'];
                    $db->query("UPDATE `cms_ads` SET `mesto` = '$mesto2' WHERE `id` = '$id'");
                    $db->query("UPDATE `cms_ads` SET `mesto` = '$mesto' WHERE `id` = '$id2'");
                }
            }
        }
    ...
    case 'del':
        ...
        */
        if ($id) {
            if (isset($_POST['submit'])) {
                $db->query("DELETE FROM `cms_ads` WHERE `id` = '$id'");
    ...
    case 'clear':
        ...
        */
        if (isset($_POST['submit'])) {
            $db->query("DELETE FROM `cms_ads` WHERE `to` = '1'");
    ...
    case 'show':
        ...
        */
        if ($id) {
            ...
            if ($req->num_rows) {
                $res = $req->fetch_assoc();
                $db->query("UPDATE `cms_ads` SET `to`='" . ($res['to'] ? 0 : 1) . "' WHERE `id` = '$id'");
            }
        }
    ...
    default:
        ...
        */
        ...
        $total = $db->query("SELECT COUNT(*) FROM `cms_ads` WHERE `type` = '$type'")->fetch_row()[0];
        if ($total) {
