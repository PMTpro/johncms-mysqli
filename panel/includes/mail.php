...
if (isset($_POST['submit'])) {
    ...
    */
	...
	$db->query("UPDATE `cms_settings` SET `val`='" . $db->real_escape_string($_POST['them_message']) . "' WHERE `key` = 'them_message'");
    $db->query("UPDATE `cms_settings` SET `val`='" . $db->real_escape_string($_POST['reg_message']) . "' WHERE `key` = 'reg_message'");
    $db->query("UPDATE `cms_settings` SET `val`='" . (isset($_POST['message_include']) && $_POST['message_include'] == 1 ? 1 : 0) . "' WHERE `key` = 'setting_mail'");
	$db->query("UPDATE `cms_settings` SET `val`='" . $db->real_escape_string(serialize($set_mail)) . "' WHERE `key` = 'setting_mail'");
