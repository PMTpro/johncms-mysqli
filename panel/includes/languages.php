...
if(isset($_GET['refresh'])){
    $db->query("DELETE FROM `cms_settings` WHERE `key` = 'lng_list'");
    core::$lng_list = array();
...
if (!empty($lng_add) || !empty($lng_del)) {
    if (!empty($lng_del) && in_array($set['lng'], $lng_del)) {
        ...
        $db->query("UPDATE `cms_settings` SET `val` = '" . key($lng_list[]) . "' WHERE `key` = 'lng' LIMIT 1");
    }
    $req = $db->query("SELECT * FROM `cms_settings` WHERE `key` = 'lng_list'");
    if ($req->num_rows) {
        $db->query("UPDATE `cms_settings` SET `val` = '" . $db->real_escape_string(serialize($lng_list)) . "' WHERE `key` = 'lng_list' LIMIT 1");
    } else {
        $db->query("INSERT INTO `cms_settings` SET `key` = 'lng_list', `val` = '" . $db->real_escape_string(serialize($lng_list)) . "'");
    }
}
    ...
    case 'set':
        ...
        */
        ...
        if ($iso && array_key_exists($iso, $lng_list)) {
            $db->query("UPDATE `cms_settings` SET `val` = '" . $db->real_escape_string($iso) . "' WHERE `key` = 'lng'");
        }
