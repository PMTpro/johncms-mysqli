...
switch ($mod) {
    case 'approve':
        ...
        */
        ...
        @$db->query("UPDATE `users` SET `preg` = '1', `regadm` = '$login' WHERE `id` = '$id'");
    ...
    case 'massapprove':
        ...
        -----------------------------------------------------------------
        */
        $db->query("UPDATE `users` SET `preg` = '1', `regadm` = '$login' WHERE `preg` = '0'");
        echo '<div class="menu"><p>' . $lng['reg_approved'] . '<br /><a href="index.php?act=reg">' . $lng['continue'] . '</a></p></div>';
        break;
    ...
    case 'del':
        ...
        */
        ...
        if ($req->num_rows) {
            ...
            $db->query("DELETE FROM `cms_users_iphistory` WHERE `user_id` = '$id' LIMIT 1");
        }
    ...
    case 'massdel':
        ...
        -----------------------------------------------------------------
        */
        $req = $db->query("SELECT `id` FROM `users` WHERE `preg` = '0'");
        while ($res = $req->fetch_assoc()) {
            $db->query("DELETE FROM `cms_users_iphistory` WHERE `user_id` = '" . $res['id'] . "'");
        }
        $db->query("DELETE FROM `users` WHERE `preg` = '0'");
    ...
    case 'delip':
        ...
        */
        ...
        if ($ip) {
            ...
            $db->query("DELETE FROM `users` WHERE `preg` = '0' AND `ip` = '$ip'");
            $db->query("OPTIMIZE TABLE `cms_users_iphistory` , `users`");
    ...
    default:
        ...
        -----------------------------------------------------------------
        */
        $total = $db->query("SELECT COUNT(*) FROM `users` WHERE `preg` = '0'")->fetch_row()[0];
