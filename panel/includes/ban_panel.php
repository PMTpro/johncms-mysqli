...
switch ($mod) {
            ...
            if (isset($_POST['submit'])) {
                ...
                if ($term) {
                    ...
                    while ($res = $req->fetch_array()) {
                        ...
                        if ($ban_left < 2592000) {
                            $amnesty_msg = isset($lng_ban['amnesty']) ? $db->real_escape_string($lng_ban['amnesty']) : 'Amnesty';
                            $db->query("UPDATE `cms_ban_users` SET `ban_time`='" . time() . "', `ban_raz`='--$amnesty_msg--' WHERE `id` = '" . $res['id'] . "'");
                        }
                    }
    ...
    default:
        ...
        */
        ...
        $req = $db->query("SELECT `user_id` FROM `cms_ban_users` GROUP BY `user_id`");
        $total = $req->num_rows;
        $req = $db->query("SELECT COUNT(`cms_ban_users`.`user_id`) AS `bancount`, MAX(`cms_ban_users`.`ban_time`) AS `bantime`, `cms_ban_users`.`id` AS `ban_id`, `users`.*
