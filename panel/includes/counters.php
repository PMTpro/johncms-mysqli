        ...
        if ($id) {
            ...
            if ($req->num_rows) {
                if (isset($_GET['go']) && $_GET['go'] == 'on') {
                    $db->query("UPDATE `cms_counters` SET `switch` = '1' WHERE `id` = '$id'");
                    $req = $db->query("SELECT * FROM `cms_counters` WHERE `id` = '$id'");
                } elseif (isset($_GET['go']) && $_GET['go'] == 'off') {
                    $db->query("UPDATE `cms_counters` SET `switch` = '0' WHERE `id` = '$id'");
                    $req = $db->query("SELECT * FROM `cms_counters` WHERE `id` = '$id'");
                }
                $res = $req->fetch_array();
        ...
        if ($id) {
            ...
            if ($req->num_rows) {
                ...
                if ($req->num_rows) {
                    ...
                    $id2 = $res['id'];
                    $sort2 = $res['sort'];
                    $db->query("UPDATE `cms_counters` SET `sort` = '$sort2' WHERE `id` = '$id'");
                    $db->query("UPDATE `cms_counters` SET `sort` = '$sort' WHERE `id` = '$id2'");
                }
            }
        }
        ...
        */
        ...
        if ($req->num_rows) {
            if (isset($_POST['submit'])) {
                $db->query("DELETE FROM `cms_counters` WHERE `id` = '$id'");
    ...
    case 'add':
        ...
        */
        ...
        if ($id) {
            ...
            $db->query("UPDATE `cms_counters` SET
            `name` = '" . functions::check($name) . "',
            `link1` = '" . $db->real_escape_string($link1) . "',
            `link2` = '" . $db->real_escape_string($link2) . "',
            `mode` = '$mode'
            WHERE `id` = '$id'");
        } else {
            ...
            // Режим добавления
            $db->query("INSERT INTO `cms_counters` SET
            `name` = '" . functions::check($name) . "',
            `sort` = '$sort',
            `link1` = '" . $db->real_escape_string($link1) . "',
            `link2` = '" . $db->real_escape_string($link2) . "',
