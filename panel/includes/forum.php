...
switch ($mod) {
    case 'del':
        ...
        */
        ...
        if ($req->num_rows) {
            ...
            $total = $db->query("SELECT COUNT(*) FROM `forum` WHERE `refid` = '$id' AND (`type` = 'f' OR `type` = 'r' OR `type` = 't')")->fetch_row()[0];
            if ($total) {
                if ($res['type'] == 'f') {
                    ////////////////////////////////////////////////////////////
                    ...
                    if (isset($_POST['submit'])) {
                        ...
                        $check = $db->query("SELECT COUNT(*) FROM `forum` WHERE `id` = '$category' AND `type` = 'f'")->fetch_row()[0];
                        if (!$check) {
                            echo functions::display_error($lng['error_wrong_data']);
                            require('../incfiles/end.php');
                            exit;
                        ...
                        $sort = $db->query("SELECT * FROM `forum` WHERE `refid` = '$category' AND `type` ='r' ORDER BY `realid` DESC")->fetch_assoc();
                        $sortnum = !empty($sort['realid']) && $sort['realid'] > 0 ? $sort['realid'] + 1 : 1;
                        $req_c = $db->query("SELECT * FROM `forum` WHERE `refid` = '$id' AND `type` = 'r'");
                        while ($res_c = $req_c->fetch_assoc()) {
                            $db->query("UPDATE `forum` SET `refid` = '" . $category . "', `realid` = '$sortnum' WHERE `id` = '" . $res_c['id'] . "'");
                            ++$sortnum;
                        }
                        // Перемещаем файлы в выбранную категорию
                        $db->query("UPDATE `cms_forum_files` SET `cat` = '" . $category . "' WHERE `cat` = '" . $res['refid'] . "'");
                        ...
                        while ($res_c = $req_c->fetch_assoc()) echo '<option value="' . $res_c['id'] . '">' . $res_c['text'] . '</option>';
                    ...
                    if (isset($_POST['submit'])) {
                        ...
                        $check = $db->query("SELECT COUNT(*) FROM `forum` WHERE `id` = '$subcat' AND `type` = 'r'")->fetch_row()[0];
                        if (!$check) {
                            echo functions::display_error($lng['error_wrong_data'], '<a href="index.php?act=forum">' . $lng_forum['forum_management'] . '</a>');
                            require('../incfiles/end.php');
                            exit;
                        }
                        $db->query("UPDATE `forum` SET `refid` = '$subcat' WHERE `refid` = '$id'");
                        $db->query("UPDATE `cms_forum_files` SET `subcat` = '$subcat' WHERE `subcat` = '$id'");
                        $db->query("DELETE FROM `forum` WHERE `id` = '$id'");
                        ...
                        while ($res_f = $req_f->fetch_assoc()) {
                            unlink('../files/forum/attach/' . $res_f['filename']);
                        }
                        $db->query("DELETE FROM `cms_forum_files` WHERE `subcat` = '$id'");
                        // Удаляем посты, голосования и метки прочтений
                        $req_t = $db->query("SELECT `id` FROM `forum` WHERE `refid` = '$id' AND `type` = 't'");
                        while ($res_t = $req_t->fetch_assoc()) {
                            $db->query("DELETE FROM `forum` WHERE `refid` = '" . $res_t['id'] . "'");
                            $db->query("DELETE FROM `cms_forum_vote` WHERE `topic` = '" . $res_t['id'] . "'");
                            $db->query("DELETE FROM `cms_forum_vote_users` WHERE `topic` = '" . $res_t['id'] . "'");
                            $db->query("DELETE FROM `cms_forum_rdm` WHERE `topic_id` = '" . $res_t['id'] . "'");
                        }
                        // Удаляем темы
        ...
        */
        ...
        if (isset($_POST['submit'])) {
            ...
            if (!$error) {
                ...
                if ($db->query("INSERT INTO `forum` SET
                `refid` = '" . ($id ? $id : 0) . "',
                `type` = '" . ($id ? 'r' : 'f') . "',
                `text` = '$name',
                `soft` = '$desc',
        ...
        if ($req->num_rows) {
            ...
            if ($res['type'] == 'f' || $res['type'] == 'r') {
                if (isset($_POST['submit'])) {
                    ...
                    elseif ($res['type'] == 'r' && !$db->query("SELECT COUNT(*) FROM `forum` WHERE `id` = '$category' AND `type` = 'f'")->fetch_row()[0])
                        $error[] = $lng_forum['error_category_select'];
                    if (!$name)
                        $error[] = $lng['error_empty_title'];
                    if ($name && (mb_strlen($name) < 2 || mb_strlen($name) > 30))
                    ...
                    if (!$error) {
                        ...
                        if ($res['type'] == 'r' && $category != $res['refid']) {
                            ...
                            $res_s = $req_s->fetch_assoc();
                            $sort = $res_s['realid'] + 1;
                            // Меняем категорию
                            $db->query("UPDATE `forum` SET `refid` = '$category', `realid` = '$sort' WHERE `id` = '$id'");
        ...
        if ($id) {
            ...
            if ($req->num_rows) {
                ...
                if ($req->num_rows) {
                    ...
                    $id2 = $res['id'];
                    $sort2 = $res['realid'];
                    $db->query("UPDATE `forum` SET `realid` = '$sort2' WHERE `id` = '$id'");
                    $db->query("UPDATE `forum` SET `realid` = '$sort' WHERE `id` = '$id2'");
                }
            }
        }
        ...
        if ($id) {
            ...
            while ($res = $req->fetch_assoc()) {
                ...
                echo '<a href="index.php?act=forum&amp;mod=cat&amp;id=' . $res['id'] . '"><b>' . $res['text'] . '</b></a> ' .
                    '(' . $db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 'r' AND `refid` = '" . $res['id'] . "'")->fetch_row()[0] . ')' .
                    '&#160;<a href="../forum/index.php?id=' . $res['id'] . '">&gt;&gt;</a>';
                if (!empty($res['soft']))
                    echo '<br /><span class="gray"><small>' . $res['soft'] . '</small></span><br />';
                echo '<div class="sub">' .
        ...
        if (isset($_POST['deltopic'])) {
            ...
            while ($res = $req->fetch_assoc()) {
                ...
                if ($req_f->num_rows) {
                    ...
                    $db->query("DELETE FROM `cms_forum_files` WHERE `topic` = '" . $res['id'] . "'");
                }
                // Удаляем посты
                $db->query("DELETE FROM `forum` WHERE `type` = 'm' AND `refid` = '" . $res['id'] . "'");
            }
            // Удаляем темы
            $req = $db->query("DELETE FROM `forum` WHERE `type` = 't' AND `close` = '1' $sort");
            header('Location: index.php?act=forum&mod=htopics');
        } else {
            $total = $db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 't' AND `close` = '1' $sort")->fetch_row()[0];
            ...
            if ($req->num_rows) {
                $i = 0;
                while ($res = $req->fetch_assoc()) {
                    $subcat = $db->query("SELECT * FROM `forum` WHERE `id` = '" . $res['refid'] . "'")->fetch_assoc();
                    $cat = $db->query("SELECT * FROM `forum` WHERE `id` = '" . $subcat['refid'] . "'")->fetch_assoc();
        ...
        if (isset($_POST['delpost'])) {
            ...
            while ($res = $req->fetch_assoc()) {
                ...
                if ($req_f->num_rows) {
                    ...
                    // Удаляем файлы
                    unlink('../files/forum/attach/' . $res_f['filename']);
                    $db->query("DELETE FROM `cms_forum_files` WHERE `post` = '" . $res['id'] . "' LIMIT 1");
                }
            }
            // Удаляем посты
            $db->query("DELETE FROM `forum` WHERE `type` = 'm' AND `close` = '1' $sort");
            header('Location: index.php?act=forum&mod=hposts');
        } else {
            $total = $db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 'm' AND `close` = '1' $sort")->fetch_row()[0];
            ...
            if ($req->num_rows) {
                ...
                while ($res = $req->fetch_assoc()) {
                    ...
                    $page = ceil($db->query("SELECT COUNT(*) FROM `forum` WHERE `refid` = '" . $res['refid'] . "' AND `id` " . ($set_forum['upfp'] ? ">=" : "<=") . " '" . $res['fid'] . "'")->fetch_row()[0] / $kmess);
                    $text = mb_substr($res['text'], 0, 500);
                    $text = functions::checkout($text, 1, 0);
                    $text = preg_replace('#\[c\](.*?)\[/c\]#si', '<div class="quote">\1</div>', $text);
                    $theme = $db->query("SELECT `id`, `text` FROM `forum` WHERE `id` = '" . $res['refid'] . "'")->fetch_assoc();
    ...
    default:
        ...
        -----------------------------------------------------------------
        */
        $total_cat = $db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 'f'")->fetch_row()[0];
        $total_sub = $db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 'r'")->fetch_row()[0];
        $total_thm = $db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 't'")->fetch_row()[0];
        $total_thm_del = $db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 't' AND `close` = '1'")->fetch_row()[0];
        $total_msg = $db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 'm'")->fetch_row()[0];
        $total_msg_del = $db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 'm' AND `close` = '1'")->fetch_row()[0];
        $total_files = $db->query("SELECT COUNT(*) FROM `cms_forum_files`")->fetch_row()[0];
        $total_votes = $db->query("SELECT COUNT(*) FROM `cms_forum_vote` WHERE `type` = '1'")->fetch_row()[0];
