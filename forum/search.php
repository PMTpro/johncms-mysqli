        ...
        */
        if (core::$user_id) {
            if (isset($_POST['submit'])) {
                $db->query("DELETE FROM `cms_users_data` WHERE `user_id` = '" . core::$user_id . "' AND `key` = 'forum_search' LIMIT 1");
        ...
        */
        ...
        if ($search && !$error) {
            ...
            */
            ...
            $count = count($array);
            $query = $db->real_escape_string($search);
            $total = $db->query("
            ...
            if ($total) {
                ...
                while ($res = $req->fetch_assoc()) {
                    ...
                    if (!$search_t) {
                        ...
                        $req_t = $db->query("SELECT `id`,`text` FROM `forum` WHERE `id` = '" . $res['refid'] . "'");
                        $res_t = $req_t->fetch_assoc();
                        echo '<b>' . $res_t['text'] . '</b><br />';
                    } else {
                        // Поиск в названиях тем
                        $req_p = $db->query("SELECT `text` FROM `forum` WHERE `refid` = '" . $res['id'] . "' ORDER BY `id` ASC LIMIT 1");
                        $res_p = $req_p->fetch_assoc();
        ...
        if (core::$user_id) {
            ...
            if ($req->num_rows) {
                ...
                if ($to_history && !in_array($search, $history)) {
                    ...
                    $history[] = $search;
                    $db->query("UPDATE `cms_users_data` SET
                        `val` = '" . $db->real_escape_string(serialize($history)) . "'
                ...
                $history[] = $search;
                $db->query("INSERT INTO `cms_users_data` SET
