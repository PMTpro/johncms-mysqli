...
$exist = $db->query("SELECT COUNT(*) FROM `cms_forum_files` WHERE `post` = '$id'")->fetch_row()[0];
if ($exist) {
    ...
    */
    if ($do_file) {
        ...
        if (!$error) {
            ...
            $req2 = $db->query("SELECT * FROM `forum` WHERE `id` = '" . $res['refid'] . "'");
            $res2 = $req2->fetch_array();
            $req3 = $db->query("SELECT * FROM `forum` WHERE `id` = '" . $res2['refid'] . "'");
            $res3 = $req3->fetch_array();

            // Заносим данные в базу
            $db->query("INSERT INTO `cms_forum_files` SET
                        `cat` = '" . $res3['refid'] . "',
                        `subcat` = '" . $res2['refid'] . "',
                        `topic` = '" . $res['refid'] . "',
                        `post` = '$id',
                        `time` = '" . $res['time'] . "',
                        `filename` = '" . $db->real_escape_string($fname) . "',
                        `filetype` = '$type'
                    ");
        } else {
    ...
    $pa = $db->query("SELECT `id` FROM `forum` WHERE `type` = 'm' AND `refid` = '" . $res['refid'] . "'");
    $pa2 = $pa->num_rows;
    $page = ceil($pa2 / $kmess);
