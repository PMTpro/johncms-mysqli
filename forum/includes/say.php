...
switch ($type1['type']) {
        ...
        */
        ...
        if (isset($_POST['submit'])
            ...
            $db->query("INSERT INTO `forum` SET
                ...
                `soft` = '" . $db->real_escape_string($agn1) . "',
            ...
            $page = $set_forum['upfp'] ? 1 : ceil($db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 'm' AND `refid` = '$id'" . ($rights >= 7 ? '' : " AND `close` != '1'"))->fetch_row()[0] / $kmess);
    ...
    case 'm':
        ...
        */
        $th = $type1['refid'];
        $th2 = $db->query("SELECT * FROM `forum` WHERE `id` = '$th'");
        $th1 = $th2->fetch_array();
        ...
        if (isset($_POST['submit'])
            ...
            ");
            // Вычисляем, на какую страницу попадает добавляемый пост
            $page = $set_forum['upfp'] ? 1 : ceil(mysql_result($db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 'm' AND `refid` = '$th'" . ($rights >= 7 ? '' : " AND `close` != '1'")), 0) / $kmess);
            if (isset($_POST['addfiles'])) {
                header("Location: index.php?id=$fadd&act=addfile");
            } else {
