...
if ($do || isset($_GET['new'])) {
    ...
    */
    $total = $db->query("SELECT COUNT(*) FROM `cms_forum_files` WHERE " . (isset($_GET['new'])
                                              ? " `time` > '$new'" : " `filetype` = '$do'") . $sql)->fetch_row()[0];
    if ($total > 0) {
        // Заголовок раздела
        ...
        for($i = 0; $res = $req->fetch_assoc(); ++$i){
            $req_u = $db->query("SELECT `id`, `name`, `sex`, `rights`, `lastdate`, `status`, `datereg`, `ip`, `browser` FROM `users` WHERE `id` = '" . $res['user_id'] . "'");
            ...
            $fls = round($fls / 1024->fetch_row()[0];
