...
if ($rights == 3 || $rights >= 6) {
    ...
    $typ = $db->query("SELECT * FROM `forum` WHERE `id` = '$id'");
    $ms = $typ->fetch_assoc();
    if ($ms[type] != "t") {
        require('../incfiles/head.php');
        echo functions::display_error($lng['error_wrong_data']);
    ...
    if (isset($_POST['submit'])) {
        ...
        $pt = $db->query("SELECT * FROM `forum` WHERE `type` = 't' AND `refid` = '" . $ms['refid'] . "' and text='$nn' LIMIT 1");
        if ($pt->num_rows != 0) {
            require('../incfiles/head.php');
        ...
        $db->query("update `forum` set  text='" . $nn . "' where id='" . $id . "';");
