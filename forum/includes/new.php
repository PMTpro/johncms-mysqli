        ...
        case 'reset':
            ...
            */
            ...
            while ($res = $req->fetch_array()) {
                $db->query("UPDATE `cms_forum_rdm` SET
                    `time` = '" . time() . "'
        ...
        case 'period':
            ...
            */
            ...
            if ($count > 0) {
                ...
                for ($i = 0; $res = $req->fetch_array(); ++$i) {
                    echo $i % 2 ? '<div class="list2">' : '<div class="list1">';
                    $q3 = $db->query("SELECT `id`, `refid`, `text` FROM `forum` WHERE `type`='r' AND `id`='" . $res['refid'] . "'");
                    $razd = $q3->fetch_array();
                    $q4 = $db->query("SELECT `text` FROM `forum` WHERE `type`='f' AND `id`='" . $razd['refid'] . "'");
                    $frm = $q4->fetch_array();
                    $colmes = $db->query("SELECT * FROM `forum` WHERE `refid` = '" . $res['id'] . "' AND `type` = 'm'" . ($rights >= 7 ? '' : " AND `close` != '1'") . " ORDER BY `time` DESC");
                    $colmes1 = $colmes->num_rows;
                    $cpg = ceil($colmes1 / $kmess);
                    $nick = $colmes->fetch_array();
    ...
    */
    ...
    if ($req->num_rows) {
        for ($i = 0; $res = $req->fetch_assoc(); ++$i) {
            $q3 = $db->query("select `id`, `refid`, `text` from `forum` where type='r' and id='" . $res['refid'] . "' LIMIT 1");
            $razd = $q3->fetch_assoc();
            $q4 = $db->query("select `id`, `refid`, `text` from `forum` where type='f' and id='" . $razd['refid'] . "' LIMIT 1");
            $frm = $q4->fetch_assoc();
            $nikuser = $db->query("SELECT `from`, `time` FROM `forum` WHERE `type` = 'm' AND `close` != '1' AND `refid` = '" . $res['id'] . "'ORDER BY `time` DESC");
            $colmes1 = $nikuser->num_rows;
            $cpg = ceil($colmes1 / $kmess);
