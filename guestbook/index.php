...
switch ($act) {
    case 'delpost':
        ...
        */
        if ($rights >= 6 && $id) {
            if (isset($_GET['yes'])) {
                $db->query("DELETE FROM `guest` WHERE `id`='" . $id . "'");
    ...
    case 'say':
        ...
        */
        ...
        $from = $user_id ? $login : $db->real_escape_string($name);
        // Транслит сообщения
        if ($trans)
        ...
        if ($user_id) {
            ...
            // Антифлуд для гостей
            $req = $db->query("SELECT `time` FROM `guest` WHERE `ip` = '$ip' AND `browser` = '" . $db->real_escape_string($agn) . "' AND `time` > '" . (time() - 60) . "'");
            if ($req->num_rows) {
                $res = $req->fetch_assoc();
                $flood = time() - $res['time'];
            }
        ...
        if (!$error) {
            ...
            $db->query("INSERT INTO `guest` SET
                `adm` = '$admset',
                `time` = '" . time() . "',
            ...
            if ($user_id) {
                ...
                $db->query("UPDATE `users` SET `postguest` = '$postguest', `lastpost` = '" . time() . "' WHERE `id` = '$user_id'");
            }
            header('location: index.php');
        } else {
        ...
        */
        if ($rights >= 6 && $id) {
            if (isset($_POST['submit'])
                ...
                $db->query("UPDATE `guest` SET
                    `admin` = '$login',
                    `otvet` = '" . $db->real_escape_string($reply) . "',
                    `otime` = '" . time() . "'
                    WHERE `id` = '$id'
                ");
                header("location: index.php");
        ...
        */
        if ($rights >= 6 && $id) {
            if (isset($_POST['submit'])
                ...
                $req = $db->query("SELECT `edit_count` FROM `guest` WHERE `id`='$id'");
                $res = $req->fetch_array();
            ...
            if (isset($_POST['submit'])) {
                ...
                switch ($cl) {
                    case '1':
                        ...
                        $db->query("DELETE FROM `guest` WHERE `adm`='$adm' AND `time` < '" . (time() - 86400) . "'");
                        echo '<p>' . $lng['clear_day_ok'] . '</p>';
                        break;

                    case '2':
                        // Проводим полную очистку
                        $db->query("DELETE FROM `guest` WHERE `adm`='$adm'");
                        echo '<p>' . $lng['clear_full_ok'] . '</p>';
                        break;
                    default :
                        // Чистим сообщения, старше 1 недели
                        $db->query("DELETE FROM `guest` WHERE `adm`='$adm' AND `time`<='" . (time() - 604800) . "';");
    ...
    default:
        ...
        */
        ...
        if (isset($_SESSION['ga']) && $rights >= "1") {
            $req = $db->query("SELECT COUNT(*) FROM `guest` WHERE `adm`='1'");
        } else {
            $req = $db->query("SELECT COUNT(*) FROM `guest` WHERE `adm`='0'");
        }
        $total = $db->query("SELECT COUNT(*) FROM `guest` WHERE `adm`='" . (isset($_SESSION['ga']) ? 1 : 0) . "'")->fetch_row()[0];
