...
if (isset($_GET['del'])) {
    if ($id) {
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($q->num_rows == 0) {
                ...
                $db->query("UPDATE `cms_contact` SET `ban`='0' WHERE `user_id`='$user_id' AND `from_id`='$id' AND `ban`='1';");
        ...
        if (isset($_POST['submit'])) {
            if ($res['rights'] > $rights) {
                ...
                if ($q->num_rows == 0) {
                    ...
                    $db->query("UPDATE `cms_contact` SET `ban`='1', `friends`='0', `type`='1' WHERE `user_id`='$user_id' AND `from_id`='$id';");
                    $db->query("UPDATE `cms_contact` SET `friends`='0', `type`='1' WHERE `user_id`='$id' AND `from_id`='$user_id';");
                }
    ...
    $total = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `user_id` = '" . $user_id . "' AND `ban`='1'")->fetch_row()[0];
    if ($total) {
        if ($total > $kmess) {
        ...
        for ($i = 0; $row = $req->fetch_assoc(); ++$i) {
            ...
            $count_message = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE ((`user_id`='{$row['id']}' AND `from_id`='$user_id') OR (`user_id`='$user_id' AND `from_id`='{$row['id']}')) AND `delete`!='$user_id' AND `sys`!='1' AND `spam`!='1';")->fetch_row()[0];
            $new_count_message = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE `cms_mail`.`user_id`='$user_id' AND `cms_mail`.`from_id`='{$row['id']}' AND `read`='0' AND `delete`!='$user_id' AND `sys`!='1' AND `spam`!='1';")->fetch_row()[0];
            $arg = array(
