...
if ($id) {
    ...
    $total = $db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `user_id`='$user_id' AND `from_id`='$id';")->fetch_row()[0];
    if ($total) {
        if (isset($_POST['submit'])) { //Если кнопка "Удалить" нажата
            ...
            //Удаляем контакт
            $db->query("DELETE FROM `cms_contact` WHERE `user_id`='$user_id' AND `from_id`='$id' LIMIT 1");
