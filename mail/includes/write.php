...
if ($id) {
    ...
    if ($mod == 'clear') {
        ...
        if (isset($_POST['clear'])) {
            $count_message = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE ((`user_id`='$id' AND `from_id`='$user_id') OR (`user_id`='$user_id' AND `from_id`='$id')) AND `delete`!='$user_id'")->fetch_row()[0];
            if ($count_message) {
                ...
                while ($row = $req->fetch_assoc()) {
                    if ($row['delete']) {
                        ...
                        $db->query("DELETE FROM `cms_mail` WHERE `id`='{$row['id']}' LIMIT 1");
                    } else {
                        if ($row['read'] == 0 && $row['user_id'] == $user_id) {
                            ...
                            $db->query("UPDATE `cms_mail` SET `delete` = '" . $user_id . "' WHERE `id` = '" . $row['id'] . "' LIMIT 1");
                        }
                    }
                }
            }
...
if (isset($_POST['submit']) && empty($ban['1']) && empty($ban['3']) && !functions::is_ignor($id)) {
    ...
    if (empty($error)) {
        if (!$id) {
            $query = $db->query("SELECT * FROM `users` WHERE `name_lat`='" . $db->real_escape_string($name) . "' LIMIT 1");
            if ($query->num_rows == 0) {
    ...
    if (empty($error)) {
        $rq = $db->query("SELECT * FROM `cms_mail`
        WHERE `user_id` = $user_id
        AND `from_id` = $id
        ORDER BY `id` DESC
        LIMIT 1
    ...
    if (empty($error)) {
		...
		`time` = '" . time() . "',
		`file_name` = '" . $db->real_escape_string($newfile) . "',
		`size` = '" . $sizefile . "'") or die($db->error());

        $db->query("UPDATE `users` SET `lastpost` = '" . time() . "' WHERE `id` = '$user_id';");
        if ($ch == 0) {
            $db->query("UPDATE `cms_contact` SET `time` = '" . time() . "' WHERE `user_id` = '" . $user_id . "' AND
			`from_id` = '" . $id . "';");
            $db->query("UPDATE `cms_contact` SET `time` = '" . time() . "' WHERE `user_id` = '" . $id . "' AND
			`from_id` = '" . $user_id . "';");
        }
...
if ($id) {
    ...
    $total = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE ((`user_id`='$id' AND `from_id`='$user_id') OR (`user_id`='$user_id' AND `from_id`='$id')) AND `sys`!='1' AND `delete`!='$user_id' AND `spam`='0'")->fetch_row()[0];
    ...
    if ($total) {
        ...
        if ($mass_read) {
            ...
            $db->query("UPDATE `cms_mail` SET `read`='1' WHERE `from_id`='$user_id' AND `id` IN (" . $result . ")");
        }
    } else {
