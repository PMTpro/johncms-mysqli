...
if ($id) {
    ...
    if (isset($_POST['submit'])) { //Если кнопка "Подвердить" нажата
        ...
        if ($res['sys']) {
            $db->query("DELETE FROM `cms_mail` WHERE `from_id`='$user_id' AND `id` = '$id' AND `sys`='1' LIMIT 1");
            ...
            if ($res['read'] == 0 && $res['user_id'] == $user_id) {
                ...
                $db->query("DELETE FROM `cms_mail` WHERE `user_id`='$user_id' AND `id` = '$id' LIMIT 1");
            } else {
                ...
                if ($res['delete']) {
                    ...
                    if ($res['file_name'])
                        @unlink('../files/mail/' . $res['file_name']);
                    $db->query("DELETE FROM `cms_mail` WHERE (`user_id`='$user_id' OR `from_id`='$user_id') AND `id` = '$id' LIMIT 1");
                } else {
                    $db->query("UPDATE `cms_mail` SET `delete` = '$user_id' WHERE `id` = '$id' LIMIT 1");
                }
            }
            echo '<div class="gmenu">' . $lng_mail['messages_delete_ok'] . '</div>';
