...
if ($mod == 'clear') {
    if (isset($_POST['clear'])) {
        $count_message = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE `from_id`='$user_id' AND `sys`='1';")->fetch_row()[0];
        if ($count_message) {
            ...
            if ($mass_del) {
                ...
                $db->query("DELETE FROM `cms_mail` WHERE `id` IN (" . $result . ")");
            }
        }
    ...
    $total = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE `from_id`='$user_id' AND `sys`='1' AND `delete`!='$user_id';")->fetch_row()[0];
    if ($total) {
        ...
        if ($mass_read) {
            ...
            $db->query("UPDATE `cms_mail` SET `read`='1' WHERE `from_id`='$user_id' AND `sys`='1' AND `id` IN (" . $result . ")");
        }
    } else {
