...
class comments
{
        ...
        switch ($mod) {
            case 'reply':
                ...
                */
                if ($this->item && $this->access_reply && !$this->ban) {
                    ...
                    $req = core::$db->query("SELECT * FROM `" . $this->comments_table . "` WHERE `id` = '" . $this->item . "' AND `sub_id` = '" . $this->sub_id . "' LIMIT 1");
                    if ($req->num_rows) {
                        $res = $req->fetch_assoc();
                        ...
                        if (!empty($res['reply']) && $attributes['reply_rights'] > $this->rights) {
                            ...
                            if (empty($message['error'])) {
                                ...
                                core::$db->query("UPDATE `" . $this->comments_table . "` SET
                                    `reply` = '" . core::$db->real_escape_string($message['text']) . "',
                                    `attributes` = '" . core::$db->real_escape_string(serialize($attributes)) . "'
                                    WHERE `id` = '" . $this->item . "'
                                ");
                    ...
                    if ($req->num_rows) {
                        ...
                        if ($user['rights'] > core::$user_rights) {
                            ...
                            if (empty($message['error'])) {
                                ...
                                core::$db->query("UPDATE `" . $this->comments_table . "` SET
                                    `text` = '" . core::$db->real_escape_string($message['text']) . "',
                                    `attributes` = '" . core::$db->real_escape_string(serialize($attributes)) . "'
                                    WHERE `id` = '" . $this->item . "'
                                ");
                                header('Location: ' . str_replace('&amp;', '&', $this->url));
                    ...
                    if (isset($_GET['yes'])) {
                        ...
                        if ($req->num_rows) {
                            ...
                            if (isset($_GET['all'])) {
                                ...
                                $count = core::$db->query("SELECT COUNT(*) FROM `" . $this->comments_table . "` WHERE `sub_id` = '" . $this->sub_id . "' AND `user_id` = '" . $res['user_id'] . "'")->fetch_row()[0];
                                core::$db->query("DELETE FROM `" . $this->comments_table . "` WHERE `sub_id` = '" . $this->sub_id . "' AND `user_id` = '" . $res['user_id'] . "'");
                            } else {
                                // Удаляем отдельный комментарий
                                $count = 1;
                                core::$db->query("DELETE FROM `" . $this->comments_table . "` WHERE `id` = '" . $this->item . "'");
                            }
                            // Вычитаем баллы из статистики пользователя
                            $req_u = core::$db->query("SELECT * FROM `users` WHERE `id` = '" . $res['user_id'] . "'");
                            if ($req_u->num_rows) {
                                $res_u = $req_u->fetch_assoc();
                                $count = $res_u['komm'] > $count ? $res_u['komm'] - $count : 0;
                                core::$db->query("UPDATE `users` SET `komm` = '$count' WHERE `id` = '" . $res['user_id'] . "'");
                            }
            ...
            default:
                ...
                */
                ...
                if ($this->total) {
                    $req = core::$db->query("SELECT `" . $this->comments_table . "`.*, `" . $this->comments_table . "`.`id` AS `subid`, `users`.`rights`, `users`.`lastdate`, `users`.`sex`, `users`.`status`, `users`.`datereg`, `users`.`id`
    ...
    */
    private function add_comment($message)
    {
        ...
        // Записываем комментарий в базу
        core::$db->query("INSERT INTO `" . $this->comments_table . "` SET
            `sub_id` = '" . intval($this->sub_id) . "',
            `user_id` = '" . $this->user_id . "',
            `text` = '" . core::$db->real_escape_string($message) . "',
            `reply` = '',
            `time` = '" . time() . "',
            `attributes` = '" . core::$db->real_escape_string(serialize($attributes)) . "'
        ");
        // Обновляем статистику пользователя
        core::$db->query("UPDATE `users` SET `komm` = '" . (++core::$user_data['komm']) . "', `lastpost` = '" . time() . "' WHERE `id` = '" . $this->user_id . "'");
        if($this->owner && $this->user_id == $this->owner){
            core::$db->query("UPDATE `users` SET `comm_old` = '" . (core::$user_data['komm']) . "' WHERE `id` = '" . $this->user_id . "'");
        }
        $this->added = true;
    }
    ...
    */
    private function msg_check($rpt_check = false)
    {
        ...
        if (!$error && $rpt_check) {
            $req = core::$db->query("SELECT * FROM `" . $this->comments_table . "` WHERE `user_id` = '" . $this->user_id . "' ORDER BY `id` DESC LIMIT 1");
            $res = $req->fetch_assoc();
    ...
    */
    ...
    {
        $total = core::$db->query("SELECT COUNT(*) FROM `" . $this->comments_table . "` WHERE `sub_id` = '" . $this->sub_id . "'")->fetch_row()[0];
        if ($update) {
            // Обновляем счетчики в таблице объекта
            core::$db->query("UPDATE `" . $this->object_table . "` SET `comm_count` = '$total' WHERE `id` = '" . $this->sub_id . "'");
        }
        return $total;
    }
}