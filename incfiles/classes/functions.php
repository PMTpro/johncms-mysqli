...
class functions extends core
{
    ...
    public static function antiflood()
    {
        ...
        switch ($af['mode']) {
            case 1:
                // Адаптивный режим
                $adm = self::$db->query("SELECT COUNT(*) FROM `users` WHERE `rights` > 0 AND `lastdate` > " . (time() - 300))->fetch_row()[0];
    ...
    {
        global $headmod;
        $req = self::$db->query("SELECT * FROM `cms_counters` WHERE `switch` = '1' ORDER BY `sort` ASC");
        if ($req->num_rows > 0) {
    ...
    public static function display_user($user = 0, $arg = array())
    {
        ...
        if ($ipinf || $lastvisit || isset($arg['sub']) && !empty($arg['sub']) || isset($arg['footer'])) {
            ...
            if ($ipinf) {
                ...
                if (isset($arg['iphist'])) {
                    $iptotal = self::$db->query("SELECT COUNT(*) FROM `cms_users_iphistory` WHERE `user_id` = '" . $user['id'] . "'")->fetch_row()[0];
    ...
    public static function get_user($id = 0)
    {
        if ($id && $id != self::$user_id) {
            $req = self::$db->query("SELECT * FROM `users` WHERE `id` = '$id'");
            if ($req->num_rows) {
    ...
    public static function is_friend($id = 0)
    {
        ...
        if (is_null($user_id) || $id != $user_id) {
            $query = self::$db->query("SELECT COUNT(*) FROM `cms_contact` WHERE `type` = '2' AND ((`from_id` = '$id' AND `user_id` = '" . self::$user_id . "') OR (`from_id` = '" . self::$user_id . "' AND `user_id` = '$id'))")->fetch_row()[0];
    ...
    public static function is_contact($id = 0)
    {
        ...
        if (is_null($user_id) || $id != $user_id) {
            $user_id = $id;
            $req_1 = self::$db->query("SELECT * FROM `cms_contact` WHERE `user_id` = '" . self::$user_id . "' AND `from_id` = '$id'");
            if ($req_1->num_rows) {
                $res_1 = $req_1->fetch_assoc();
                if ($res_1['ban'] == 1) {
                    $return = 2;
                } else {
                    $return = 1;
    ...
    public static function is_ignor($id)
    {
        ...
        if (is_null($user_id) || $id != $user_id) {
            $user_id = $id;
            $req_2 = self::$db->query("SELECT * FROM `cms_contact` WHERE `user_id` = '$id' AND `from_id` = '" . self::$user_id . "'");
            if ($req_2->num_rows) {
                $res_2 = $req_2->fetch_assoc();
                if ($res_2['ban'] == 1) {
                    $return = TRUE;
                }
            }
