...
class mainpage {
    ...
    private function news() {
        ...
        if ($this->settings['view'] > 0) {
            ...
            $req = core::$db->query("SELECT * FROM `news` WHERE `time` > '$reqtime' ORDER BY `time` DESC LIMIT " . $this->settings['quantity']);
            if ($req->num_rows > 0) {
                $i = 0;
                $news = '';
                while ($res = $req->fetch_array()) {
                    ...
                    if (!empty($res['kom']) && $this->settings['view'] != 2 && $this->settings['kom'] == 1) {
                        $mes = core::$db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 'm' AND `refid` = '" . $res['kom'] . "'");
                        $komm = $mes->fetch_row()[0] - 1;
                        if ($komm >= 0)
    ...
    private function newscount() {
        $req = core::$db->query("SELECT COUNT(*) FROM `news`");
        $res = $req->fetch_row()[0];
        return ($res > 0 ? $res : '0');
    }
    ...
    private function lastnewscount() {
        $req = core::$db->query("SELECT COUNT(*) FROM `news` WHERE `time` > '" . (time() - 259200) . "'");
        $res = $req->fetch_row()[0];
