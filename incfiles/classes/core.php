...
class core
{
    ...
    */
    private function db_connect()
    {
        ...
        if ($db->connect_error) die('Error: cannot connect to database server');
        $db->select_db($db_name) or die('Error: specified database does not exist');
    ...
    */
    ...
    {
        $req = self::$db->query("SELECT `ban_type`, `link` FROM `cms_ban_ip`
    ...
    */
    ...
    {
        $set = array();
        $req = self::$db->query("SELECT * FROM `cms_settings`");
    ...
    */
    private function authorize()
    {
        ...
        if ($user_id && $user_ps) {
            $req = self::$db->query("SELECT * FROM `users` WHERE `id` = '$user_id'");
            if ($req->num_rows) {
                ...
                if ($permit && $user_ps === $user_data['password']) {
                    ...
                    self::$db->query("UPDATE `users` SET `failed_login` = '" . ($user_data['failed_login'] + 1) . "' WHERE `id` = '" . $user_data['id'] . "'");
                    $this->user_unset();
                }
            } else {
    ...
    */
    ...
    {
        $req = self::$db->query("SELECT * FROM `cms_ban_users` WHERE `user_id` = '" . self::$user_id . "' AND `ban_time` > '" . time() . "'");
        if ($req->num_rows) {
            self::$user_rights = 0;
            while ($res = $req->fetch_row()) self::$user_ban[$res[4]] = 1;
        }
    }
    ...
    */
    private function user_ip_history()
    {
        if (self::$user_data['ip'] != self::$ip || self::$user_data['ip_via_proxy'] != self::$ip_via_proxy) {
            ...
            @self::$db->query("DELETE FROM `cms_users_iphistory`
            ...
            if (!empty(self::$user_data['ip']) && self::ip_valid(long2ip(self::$user_data['ip']))) {
                ...
                self::$db->query("INSERT INTO `cms_users_iphistory` SET
    ...
    */
    private function auto_clean()
    {
        if (self::$system_set['clean_time'] < time() - 86400) {
            self::$db->query("DELETE FROM `cms_sessions` WHERE `lastdate` < '" . (time() - 86400) . "'");
            self::$db->query("DELETE FROM `cms_users_iphistory` WHERE `time` < '" . (time() - 2592000) . "'");
            self::$db->query("UPDATE `cms_settings` SET  `val` = '" . time() . "' WHERE `key` = 'clean_time' LIMIT 1");
