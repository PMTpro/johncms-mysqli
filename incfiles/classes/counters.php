...
class counters
{
    ...
    */
    static function album()
    {
        ...
        if (file_exists($file) && filemtime($file) > (time() - 600)) {
            ...
            $album = core::$db->query("SELECT COUNT(DISTINCT `user_id`) FROM `cms_album_files`")->fetch_row()[0];
            $photo = core::$db->query("SELECT COUNT(*) FROM `cms_album_files`")->fetch_row()[0];
            $new = core::$db->query("SELECT COUNT(*) FROM `cms_album_files` WHERE `time` > '" . (time() - 259200) . "' AND `access` = '4'")->fetch_row()[0];
            $new_adm = core::$db->query("SELECT COUNT(*) FROM `cms_album_files` WHERE `time` > '" . (time() - 259200) . "' AND `access` > '1'")->fetch_row()[0];
    ...
    */
    static function downloads()
    {
        ...
        if (file_exists($file) && filemtime($file) > (time() - 900)) {
            ...
            $total = core::$db->query("SELECT COUNT(*) FROM `download` WHERE `type` = 'file'")->fetch_row()[0];
            $new = core::$db->query("SELECT COUNT(*) FROM `download` WHERE `time` > '" . (time() - 259200) . "' AND `type` = 'file'")->fetch_row()[0];
    ...
    */
    static function forum()
    {
        ...
        if (file_exists($file) && filemtime($file) > (time() - 600)) {
            ...
            $top = core::$db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 't' AND `close` != '1'")->fetch_row()[0];
            $msg = core::$db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 'm' AND `close` != '1'")->fetch_row()[0];
    ...
    */
    ...
    {
        if (core::$user_id) {
            $req = core::$db->query("SELECT COUNT(*) FROM `forum`
    ...
    */
    ...
    {
        $new = core::$db->query("SELECT COUNT(*) FROM `gallery` WHERE `time` > '" . (time() - 259200) . "' AND `type` = 'ft'")->fetch_row()[0];
        if ($mod == 0) {
            $total = core::$db->query("SELECT COUNT(*) FROM `gallery` WHERE `type` = 'ft'")->fetch_row()[0];
            $out = $total;
            if ($new > 0)
    ...
    */
    static function guestbook($mod = 0)
    {
        ...
        switch ($mod) {
            case 1:
                $count = core::$db->query("SELECT COUNT(*) FROM `guest` WHERE `adm`='0' AND `time` > '" . (time() - 86400) . "'")->fetch_row()[0];
                break;

            case 2:
                if (core::$user_rights >= 1)
                    $count = core::$db->query("SELECT COUNT(*) FROM `guest` WHERE `adm`='1' AND `time` > '" . (time() - 86400) . "'")->fetch_row()[0];
                break;

            default:
                $count = core::$db->query("SELECT COUNT(*) FROM `guest` WHERE `adm`='0' AND `time` > '" . (time() - 86400) . "'")->fetch_row()[0];
                if (core::$user_rights >= 1) {
                    $req = core::$db->query("SELECT COUNT(*) FROM `guest` WHERE `adm`='1' AND `time`>'" . (time() - 86400) . "'");
                    $count = $count . '&#160;/&#160;<span class="red"><a href="guestbook/index.php?act=ga&amp;do=set">' . $req->fetch_row()[0] . '</a></span>';
                }
        }
        return $count;
    }
    ...
    */
    static function library()
    {
        ...
        if (file_exists($file) && filemtime($file) > (time() - 3200)) {
            ...
            $total = core::$db->query("SELECT COUNT(*) FROM `library_texts` WHERE `premod` = '1'")->fetch_row()[0];
            $new = core::$db->query("SELECT COUNT(*) FROM `library_texts` WHERE `time` > '" . (time() - 259200) . "' AND `premod` = '1'")->fetch_row()[0];
            $mod = core::$db->query("SELECT COUNT(*) FROM `library_texts` WHERE `premod` = '0'")->fetch_row()[0];
    ...
    */
    static function online()
    {
        ...
        $users = core::$db->query("SELECT COUNT(*) FROM `users` WHERE `lastdate` > '" . (time() - 300) . "'")->fetch_row()[0];
        $guests = core::$db->query("SELECT COUNT(*) FROM `cms_sessions` WHERE `lastdate` > '" . (time() - 300) . "'")->fetch_row()[0];
    ...
    */
    static function users()
    {
        ...
        if (file_exists($file) && filemtime($file) > (time() - 600)) {
            ...
            $total = core::$db->query("SELECT COUNT(*) FROM `users`")->fetch_row()[0];
            $new = core::$db->query("SELECT COUNT(*) FROM `users` WHERE `datereg` > '" . (time() - 86400) . "'")->fetch_row()[0];
            file_put_contents($file, serialize(array('total' => $total, 'new' => $new)));
        }
