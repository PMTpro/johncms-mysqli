    ...
    */
    public function forum_contents() {
        ...
        if (file_exists($file) && filemtime($file) > (time() - $this->cache_forum_contents * 3600)) {
            ...
            $req = core::$db->query("SELECT * FROM `forum` WHERE `id` = '$id' AND `type` = 'r'");
            if ($req->num_rows) {
                $row = array();
                $res = $req->fetch_assoc();
                $req_t = core::$db->query("SELECT * FROM `forum` WHERE `refid` = '$id' AND `type` = 't' AND `close` != '1' ORDER BY `time` DESC LIMIT " . ($this->page * $this->links_count) . ", " . $this->links_count);
                if ($req_t->num_rows) {
                    while ($res_t = $req_t->fetch_assoc()) $row[] = '<a href="' . $set['homeurl'] . '/forum/index.php?id=' . $res_t['id'] . '">' . $res_t['text'] . '</a>';
    ...
    */
    public function library_contents() {
        ...
        if (file_exists($file) && filemtime($file) > (time() - $this->cache_lib_contents * 3600)) {
            ...
            $req = core::$db->query("SELECT * FROM `library_cats` WHERE `id` = '$id'");
            if ($req->num_rows) {
                $row = array();
                $res = $req->fetch_assoc();
                $req_a = core::$db->query("SELECT * FROM `library_texts` WHERE `cat_id` = '$id' AND `premod` = '1' ORDER BY `time` ASC LIMIT " . ($this->page * $this->links_count) . ", " . $this->links_count);
                if ($req_a->num_rows) {
                    while ($res_a = $req_a->fetch_assoc()) $row[] = '<a href="' . $set['homeurl'] . '/library/index.php?do=text&amp;id=' . $res_a['id'] . '">' . functions::checkout($res_a['name']) . '</a>';
    ...
    */
    private function forum_map() {
        ...
        if (file_exists($file) && filemtime($file) > (time() - $this->cache_forum_map * 3600)) {
            ...
            $req = core::$db->query("SELECT * FROM `forum` WHERE `type` = 'r'");
            if ($req->num_rows) {
                while ($res = $req->fetch_assoc()) {
                    $count = core::$db->query("SELECT COUNT(*) FROM `forum` WHERE `refid` = '" . $res['id'] . "' AND `type` = 't' AND `close` != '1'")->fetch_row()[0];
                    if ($count) {
    ...
    */
    private function library_map() {
        ...
        if (file_exists($file) && filemtime($file) > (time() - $this->cache_lib_map * 3600)) {
            ...
            $req = core::$db->query("SELECT * FROM `library_cats` WHERE `dir` = '0'");
            if ($req->num_rows) {
                while ($res = $req->fetch_assoc()) {
                    $count = core::$db->query("SELECT COUNT(*) FROM `library_texts` WHERE `cat_id` = '" . $res['id'] . "' AND `premod` = '1'")->fetch_row()[0];
                    if ($count) {
