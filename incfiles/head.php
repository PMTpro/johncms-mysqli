...
$headmod = isset($headmod) ? $db->real_escape_string($headmod) : '';
...
if ($user_id) {
    ...
    if ($datauser['place'] != $headmod) {
        ++$movings;
        $sql .= " `place` = '" . $db->real_escape_string($headmod) . "', ";
    }
    if ($datauser['browser'] != $agn)
        $sql .= " `browser` = '" . $db->real_escape_string($agn) . "', ";
    ...
    if ($req->num_rows) {
        ...
        $db->query("INSERT INTO `cms_sessions` SET
            ...
            `lastdate` = '" . time() . "',
            `sestime` = '" . time() . "',
            `place` = '" . $db->real_escape_string($headmod) . "'
        ");
    }
}
...
if ($user_id) {
    $list = array();
    $new_sys_mail = $db->query("SELECT COUNT(*) FROM `cms_mail` WHERE `from_id`='$user_id' AND `read`='0' AND `sys`='1' AND `delete`!='$user_id';")->fetch_row()[0];
	if ($new_sys_mail) $list[] = '<a href="' . $home . '/mail/index.php?act=systems">Система</a> (+' . $new_sys_mail . ')';
	$new_mail = $db->query("SELECT COUNT(*) FROM `cms_mail` LEFT JOIN `cms_contact` ON `cms_mail`.`user_id`=`cms_contact`.`from_id` AND `cms_contact`.`user_id`='$user_id' WHERE `cms_mail`.`from_id`='$user_id' AND `cms_mail`.`sys`='0' AND `cms_mail`.`read`='0' AND `cms_mail`.`delete`!='$user_id' AND `cms_contact`.`ban`!='1' AND `cms_mail`.`spam`='0'")->fetch_row()[0];
    ...
    $new_album_comm = $db->query("SELECT COUNT(*) FROM `cms_album_files` WHERE `user_id` = '" . core::$user_id . "' AND `unread_comments` = 1")->fetch_row()[0];
