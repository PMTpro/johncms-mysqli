...
if ($totalnew != 0) {
    while ($newf = $newfile->fetch_array()) {
        if ($i >= $start && $i < $end) {
            $d = $i / 2;
            $d1 = ceil($d);
            $d2 = $d1 - $d;
            ...
            imagedestroy($im);
            imagedestroy($im1);
            $kom = $db->query("select * from `gallery` where type='km' and refid='" . $newf['id'] . "';");
            $kom1 = $kom->num_rows;
            echo "</a><br/>" . $lng['date'] . ': ' . functions::display_date($newf['time']) . '<br/>' . $lng['description'] . ": $newf[text]<br/>";
            $al = $db->query("select * from `gallery` where type = 'al' and id = '" . $newf['refid'] . "';");
            $al1 = $al->fetch_array();
            $rz = $db->query("select * from `gallery` where type = 'rz' and id = '" . $al1['refid'] . "';");
            $rz1 = $rz->fetch_array();
